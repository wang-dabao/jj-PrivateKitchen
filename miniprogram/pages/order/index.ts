...
Component({
  ...
  lifetimes: {
    attached() {
      ...
      const orderId = options.orderId || wx.getStorageSync('currentOrderId') || ''
  ...
  methods: {
    async fetchOrder() {
      if (!this.data.orderId) {
        console.error('fetchOrder: orderId 为空')
        this.setData({ loading: false, error: '订单ID缺失' })
        return
      }
      try {
        console.log('fetchOrder: 开始获取订单', this.data.orderId)
        const order = await callFunction('getOrder', { orderId: this.data.orderId })
        console.log('fetchOrder: 获取成功', order)
        ...
        this.setData({ loading: false, error: e.errMsg || '获取订单失败' })
      }
    },
    goHome() {
